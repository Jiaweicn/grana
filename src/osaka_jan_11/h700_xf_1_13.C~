...
// special correction for 13 deg measurement. (only works for AC<0 deg)
       if( fabs(ang3-13)<0.5 ) {
// calibration based on the line at X=-120
//         Xcp = Xc-((((-1.595642*ac-1.679699)*ac + 1.972625)*ac + 15.266526)*ac);
// calibration based on the line at X=34.14
         Xcp = Xc-((((-1.07686*ac - 3.38443)*ac + 0.14257)*ac + 14.75012)*ac);
	}
...
//       Double_t brho3=b_percent*((-1.304745e-04*(Xcp/100) - 1.300470e-02)*(Xcp/100) + 2.071812e+00)/1.01961;
	 ...
	 brho=brho3;
	   ...
	   h2_Rfc_X->Fill(X,Rfc);
           Double_t ex1=relex(brho1*2*speed_of_light/1e6,ang3);
           Double_t ex2=relex(brho2*2*speed_of_light/1e6,ang3);
           Double_t ex3=relex(brho3*2*speed_of_light/1e6,ang3);
           Double_t ex4=relex(brho *2*speed_of_light/1e6,ang3);
	   Double_t ex5=relex(brho5*2*speed_of_light/1e6,ang3);

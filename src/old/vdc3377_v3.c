...
extern struct vdcdata lasfx,lasfu,lasrx,lasru;
...
int vdc_3377(){
  ...
  int mod_fr_x_l,mod_fr_u_l,mod_re_x_l,mod_re_u_l;
...
/*    printf("\tVDC channel:%d\tchannel:%d\tbase_ch:%d\n",
	...
	break;
      default:
	printf("%s:No 3377 header word exists.(%4x) b:%d e:%d\n"
...
void bookvdc(struct vdcdata *vdc, int histid){
  ...
  if(vdc->good==1){
    ...
    book1d(histid+33,vdc->dis2); /*tmp*/
    book1d(histid+96,vdc->dis2-vdc->dis); /*tmp*/
  }
}  

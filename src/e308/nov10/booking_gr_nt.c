...
histid1=400;
for(i=0;i<4;i++){
  book1(396+i,(float)issdmul[i]);
  book1(400+i,issdhitr[i]);
  book1(404+i,(issdhit[i]&0xf));
  if(fcet_id[9]!=1) book1(1400+i,issdhita[i]);
}

for(i=0;i<grssd.k_adc;i++){
  j=grssd.adcc_ch[i];
  histid2=412;
  /*book1(800+j,grssd.adc[i]);*/
  if(fcet_id[9]==1){
    book2d(408,(float)j,(float)grssd.adcc[j]);
  }
  if(fcet_id[9]!=1){
    if(j<8){
      if(grssd.adc[i]&&feret_gr[18])
	book2d(histid2,(float)j,(float)grssd.adcc[j]);
    }
    else{
      if(j<16){
	if(grssd.adc[i]&&feret_gr[19])
	  book2d(histid2,(float)j,(float)grssd.adcc[j]);
      }
      else{
	if(grssd.tdcc[j]) book2d(histid2,(float)j,(float)grssd.adcc[j]);
      }
    }
  }    
 }
...
for(i=0;i<grssd.k_tdc;i++){
  histid2=414;
  j=grssd.tdcc_ch[i];
  book2d(histid2,(float)j,(float)grssd.tof[j]);
  if(j<16) book1d(900+j,grssd.tof[j]);
  else {
    if(j%4==0){
      if((grssd.tdcc[j+1]>grssdtg[j-1][0] &&
	  grssd.tdcc[j+1]<grssdtg[j-1][1]) ||
	 ihit3[(j-16)/4]>=0)
	book1d(900+j,grssd.tof[j]);
    }
    else{
      if(grssd.tdcc[j-1]>grssdtg[j-1][0] &&
	 grssd.tdcc[j-1]<grssdtg[j-1][1])
  ...
  if(grssd.tdcc[j]>grssdtg[j][0]&&grssd.tdcc[j]<grssdtg[j][1]){
    book2d(1412,(float)j,(float)grssd.adcc[j]);
    book2d(1020+j,grssd.tof[j],grssd.adcc[j]);
  }
 }
...
for(i=0;i<4;i++){
  if(ihit3[i]==0){
    if(feret_gr[18]){
      book1(1404,issdhita[0]);
      book1(1405,issdhita[1]);
    }
    if(feret_gr[19]){
      book1(1406,issdhita[2]);
      book1(1407,issdhita[3]);
    }
  }

  if((issdhitr[i]&0x3)==0x3){
    book2d(417+i,ene_ssd[i],grssd.adcc[ihit3[i]]);
    book2d(421+i,grssd.adcc[i*4+16],grssd.adcc[ihit3[i]]);

    book2d(433+i,grssd.adcc[i*4+16],grssd.tdcc[i*4+16]);

  }
  if((issdhit[i]&0xf0)==0){
    if(issdmul[i]<=1 && ihit3[i]!=-1){
      double tof,tofe,tbeta;
      double static dtof[16][2]={
	{-30.0, 25.0}, {-30.0, 32.0}, {-30.0, 33.0}, {-25.0, 40.0},
	{-30.0, 38.0}, {-30.0, 33.0}, {-30.0, 28.0}, {-30.0, 42.0},
	{-15.0, 32.0}, {-15.0, 33.0}, {-10.0, 36.0}, {-10.0, 60.0},
	{-20.0, 25.0}, {-20.0, 40.0}, {-25.0, 33.0}, {-30.0, 26.0}};
      book2d(1000+ihit3[i],grssd.tof[ihit3[i]],grssd.adcc[ihit3[i]]);
      /*      book2d(1020+ihit3[i],grssd.tof[ihit3[i]],grssd.adcc[ihit3[i]]);*/
      /*      book2d(1020+ihit3[i],ssdtpid[ihit3[i]],grssd.adcc[ihit3[i]]);*/

      /*      book2d(1020+ihit3[i],ssdtpid[ihit3[i]],grssd.adcc[ihit3[i]]);*/
      /*      book2d(416,grssd.tdcc[ihit3[i]],grssd.adcc[ihit3[i]]);*/
      /*      book2d(416,grssd.tdcc[ihit3[i]]*0.27024-feret_gr[4]*0.12253,
	      grssd.adcc[ihit3[i]]);*/
      if(grssd.adcc[ihit3[i]]>0.2 && issdpid[i]==0){
	tbeta=sqrt(2.0*grssd.adcc[ihit3[i]]/3752);
	tofe=18.0/30.0/tbeta;
	/*
	printf("tdcc:%10.4f  rf_t:%10.4f  tofe:%10.4f  a:%10.4f\n",
	grssd.tdcc[ihit3[i]],rf_t,tofe,grssd.adcc[ihit3[i]]);*/

	if(i<2) tof=grssd.tdcc[ihit3[i]]*0.28270;
	else tof=grssd.tdcc[ihit3[i]]*0.28123;
	tof-=tofe;
	tof-=dtof[ihit3[i]][0]*
	  exp(-1.0*grssd.adcc[ihit3[i]])+dtof[ihit3[i]][1]-40.0;

	if(fabs(tof-feret_gr[4]*0.12253-40)<4.5 ||
	   fabs(tof-feret_gr[4]*0.12253-138.6)<4.5){
	  issdpid[i]=4;
	}
	if((tof-feret_gr[4]*0.12253<35.5 && tof-feret_gr[4]*0.12253>0)||
	   (tof-feret_gr[4]*0.12253<134.1 && tof-feret_gr[4]*0.12253>100))
	  issdpid[i]=1;
	book1d(800+ihit3[i],tof-feret_gr[4]*0.12253);
	book1d(816,tof-feret_gr[4]*0.12253);
      }
    }
  }
 }
...
for(channel=0;channel<32;channel++){
  if(feret_gr[channel]!=0){
    book1(1000+channel,(float)feret_gr[channel]);}
}
#endif
...
for(channel=0;channel<32;channel++){
  if(feret_gr[channel]!=0){
    book2d(411,(double)channel,(double)feret_gr[channel]);}
}
book1d(1138,rf_t);
...
/******* GR_FP_EVENT ***********/
  ...
  if(good_rf){
    ...
    /*    book2d(histid2+7,vdcpos[9][1],theta_y[0]);*/
    ...
    if(fabs(theta_x[1])<0.2 && fabs(theta_y[1])<0.2){
...
/* y-cut ***********/
	...
	if(issdpid[i]==1){
	  double fst;
	  fst=ene_ssd[i]-(23.0/24.0*(ex_energy-11.69));
	  book1d(502,ex_energy);
	  book2d(428,ex_energy,ene_ssd[i]);
	  if(fst<0.5 && fst>-1.3) book1d(444,ex_energy);
	}
	if(issdpid[i]==4){
	  double fst;
	  book1d(504,ex_energy);
	  book2d(430,ex_energy,ene_ssd[i]);
	  fst=ene_ssd[i]-(5.0/6.0*(ex_energy-9.31));
	  if(fst<0.1 && fst>-1.0) book1d(440,ex_energy);
	  if(fst<-1.0 && fst>-2.5) book1d(442,ex_energy);
	}
      }
#if 0
      for(i=0;i<4;i++){
	...
	if(issdpid[i]==1){
	  ...
	  book2d(429,ex_energy,ene_ssd[i]);
	  if(fst<0.5 && fst>-1.3) book1d(445,ex_energy);
	}
	if(issdpid[i]==4){
	  double fst;
	  fst=ene_ssd[i]-(5.0/6.0*(ex_energy-9.31));
	  book1d(505,ex_energy);
	  book2d(431,ex_energy,ene_ssd[i]);
	  if(fst<0.1 && fst>-1.0) book1d(441,ex_energy);
	  if(fst<-1.0 && fst>-2.5) book1d(443,ex_energy);
	}
      }
#if 0
      for(i=0;i<4;i++){

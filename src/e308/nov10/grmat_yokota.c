...
#if 0
  static struct mat ymat[N_ELE_Y]={
    {0x0000,   -1.09677e-01},    {0x0001,    -5.22828e+00},
    {0x0002,    7.45599e-01},    
    {0x0011,   -4.10021e-01},    {0x0012,    -1.96713e-03},
    {0x0020,    4.99256e-02},    {0x0100,    -9.95066e-02},
    {0x0101,   -1.35091e+00},    {0x0110,     4.36746e-01},
    {0x0111,    7.67006e-02},    {0x0112,     1.08365e-02},
    {0x0200,    1.52805e-01},    {0x0201,    -6.11034e-01},
    {0x1000,    1.25169e-01},    {0x1010,    -4.02447e-02},
    {0x1011,   -4.02447e-02},    
  };
#endif
...
*************************************************/
  ...
  //xc = -0.014975 - 0.014724 * newa0 + 0.997259 * xtemp; //9deg
  ...
  //  ac = (-0.0339324 + 0.00940716*btemp + 1.72675*newa0 + 0.0025076*newa0*btemp + 0.0448697*xtemp +0.0207794*xtemp*xtemp)/100*180/3.1415  - 0.195/2 * (xc-xset);
  
  yc = ytemp -0.0790032 - 5.21122*btemp + 0.356687*newa0 + 0.0702114*xtemp -0.193615*xtemp*ytemp ;   
//  yc = ytemp -0.0394481 - 5.21646*btemp + 0.356579*newa0 + 0.951963*newa0*btemp + 0.0827625*xtemp
  - 1.76231*xtemp*btemp + 0.384944*xtemp*btemp*newa0; 

...
rf_t=(feret_gr[4]-rfaz[0])/(rfaz[1]);
if(rf_t>125) rf_t-=98.27;
...
/*** ADC sorting and calibration ***/
for(i=0;i<grssd.k_adc;i++){
  channel=grssd.adc_ch[i];
  j=channel>>3;                /* VSN-71 */
  issd_rayer=(channel&0x7);    /* Layer */
  if(issd_rayer<4) issd=j;     /* Input telescope ID into issd */
  else issd=(j+7-issd_rayer)%4;

  /* Input new channel ID to histid1 */
  if(issd_rayer<4) histid1=issd*4+issd_rayer;  
  else histid1=12+issd*4+issd_rayer;

  if(histid1<16) issdhita[issd]|=(1<<issd_rayer);
...
/*** TDC sorting and calibration ***/
for(i=0;i<grssd.k_tdc;i++){
  channel=grssd.tdc_ch[i];
  issd=channel/4;        /* telescope ID */
  if(issd>=4)issd-=4;    /* telescope ID */
  issd_rayer=channel%4;
  if(channel<16) {
    histid1=issd*4+issd_rayer;
  }
  else{
    issd_rayer+=4;
  ...
  grssd.tdcc[histid1]=(grssd.tdc[i]-grssdtz[channel])/grssdta[channel];
  
  grssd.tof[histid1]=grssd.tdcc[histid1]-rf_t+100;
  
  /*  grssd.tdcc[histid1]=grssd.tdc[i]-370;*/ /* Simulate Acc. coin */

  /*** Check Hitpattern ***/
  if(grssd.tdcc[histid1]>grssdtg[histid1][0]&&
     grssd.tdcc[histid1]<grssdtg[histid1][1]&&
     grssd.adcc[histid1]!=0) {
    issdhit[issd]|=(1<<issd_rayer); /* Hit pattern in each telescope (issd) */
    /*** Through correction ***/
    if(histid1<16) {
      double tssdcor;
      issdmul[issd]++;
      ihit3[issd]=histid1;
      if(ssdpidpar[histid1][2]<1e-5) /* 1dim */
	tssdcor=(log(grssd.adcc[histid1])-ssdpidpar[histid1][0])
	  /ssdpidpar[histid1][1];
      else  /* 2dim */
	tssdcor=(-ssdpidpar[histid1][1]
		 -sqrt(ssdpidpar[histid1][1]*ssdpidpar[histid1][1]
		       -4.0*ssdpidpar[histid1][2]
		       *(ssdpidpar[histid1][0]-log(grssd.adcc[histid1]))))
	  /(2.0*ssdpidpar[histid1][2]);
      ssdtpid[histid1]=grssd.tof[histid1]-tssdcor+150;
    }
  }
 }

/* issdhit[i] ....  Hitpattern  500*4bits + 65*4bits
   issdhitr[i] .... Hitpattern  500*4bits + 65*1bit  */  
for(i=0;i<4;i++){ /* loop in issd */
  issdhitr[i]=(issdhit[i]>>4);
  issdhitr[i]=(issdhitr[i]<<1);
  issdhitr[i]&=0xfffe;
  
  /*  if(issdmul[i]<=1 && fcet_id[9]!=1) {*/  /* If one or no hit in 65 um */
  if(issdmul[i]==1 && fcet_id[9]!=1) {  /* If one hit in 65 um */
    if(issdmul[i]==1) {
      issdhitr[i]|=1;
      ene_ssd[i]=grssd.adcc[ihit3[i]];
    }
#if 0     /* Ignore low energy particle */
    else {  /* Search for Max Energy from pedestals in 65 um */
      ene_ssd[i]=grssd.adcc[i*4];
      for(j=1;j<4;j++){
	if(ene_ssd[i]<grssd.adcc[i*4+j]) {
	  ene_ssd[i]=grssd.adcc[i*4+j];
	  ihit3[i]=i*4+j;
	}
      }
    }
    ...
    switch(issdhitr[i]>>1){
    case 0xf:
      ene_ssd[i]+=grssd.adcc[i*4+3+16];
    case 0x7:
      ene_ssd[i]+=grssd.adcc[i*4+2+16];
    case 0x3:
      ene_ssd[i]+=grssd.adcc[i*4+1+16];
    case 0x1:
      ene_ssd[i]+=grssd.adcc[i*4+16];
    }
    
    if((issdhit[i]&0x10)==0x10){
/*      printf("%d: issdhit:%02x ihit3:%d\n",i,issdhit[i],ihit3[i]);*/
      if(ene_ssd[i]>8.5&&
	 grssd.adcc[ihit3[i]]>3.5&&
	 grssd.adcc[ihit3[i]]<10) issdpid[i]=4;  /* alpha */
      if(ene_ssd[i]>1.5&&grssd.adcc[ihit3[i]]<2.3) {
	double tgat[2];double x;
	x=ene_ssd[i];
#if 0 
	tgat[0]=ssdpidpar[i][0]*exp(-0.6*x)-0.02*x+ssdpidpar[i][1];
	tgat[1]=ssdpidpar[i][2]*exp(-0.6*x)-0.02*x+ssdpidpar[i][3];
	if(grssd.adcc[ihit3[i]]<tgat[0]&&grssd.adcc[ihit3[i]]>tgat[1])
	  issdpid[i]=1;  /* proton */
#endif
      }
    }
    if((issdhit[i]&0xf0)==0){
      if(grssd.tdcc[ihit3[i]]<750&&grssd.tdcc[ihit3[i]]<660&&
	 grssd.adcc[ihit3[i]]>2.5) issdpid[i]=4;
    }
